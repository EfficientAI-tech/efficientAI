...
import { EvaluationStepper, isEvaluationRunning } from '../evaluation/EvaluationProgress'
...
export default function TestVoiceAgentResultDetails({
  ...
  const MetricsCard = () => {
    const metricCount = resultData.metric_scores
      ...
      : 0
    const evaluationInProgress = isEvaluationRunning(resultData.status)

    if (metricCount === 0 && evaluationInProgress) {
      const stepStatus = resultData.status && isEvaluationRunning(resultData.status)
        ? resultData.status
        : 'queued'
      return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <EvaluationStepper status={stepStatus} />
        </div>
      )
    }
