...
import AgentsHierarchyNav, { type AgentsHierarchyCrumb } from './AgentsHierarchyNav'
import AgentWorkspaceDetail from './AgentWorkspaceDetail'
...
export default function AgentsWorkspace() {
  ...
  useEffect(() => {
    if (routeAgentId || loading || agents.length === 0) return
    const first = agents[0] as TestAgent
    const tab = searchParams.get('tab')
    const query = tab ? `?tab=${tab}` : ''
    navigate(`/agents/${agentRouteId(first)}${query}`, { replace: true })
  }, [routeAgentId, loading, agents, navigate, searchParams])

  const handleEditModeChange = useCallback((edit: boolean) => {
    setIsEditMode(edit)
  }, [])

  const buildAgentPath = (agent: TestAgent) => {
    const tab = searchParams.get('tab')
    const query = tab ? `?tab=${tab}` : ''
    return `/agents/${agentRouteId(agent)}${query}`
  }

  const handleSelectAgent = (agent: TestAgent) => {
    if (isEditMode) {
      const discard = window.confirm('Discard unsaved changes?')
      if (!discard) return
      setIsEditMode(false)
    }
    navigate(buildAgentPath(agent))
  }

  const handleSelectAgentCheckbox = (agentId: string, checked: boolean) => {
  ...
  const handleAgentDeletedFromDetail = () => {
    ...
    if (remaining.length > 0) {
      navigate(buildAgentPath(remaining[0] as TestAgent), { replace: true })
    } else {
    ...
    try {
      ...
      results.forEach((result, index) => {
        ...
        if (result.status === 'fulfilled') {
          ...
          if (routeAgentId && agentMatchesRoute(agent, routeAgentId)) {
            navigate('/agents', { replace: true })
          }
        } else {
      ...
      showToast(`Failed to delete selected agents: ${failedAgents.join(', ')}`, 'error')
      setSelectedAgents(failedIds)
      setShowBulkDeleteModal(false)
      setShowBulkImpactModal(false)
      setBulkImpactItems([])
    ...
    try {
      ...
      const detail =
        err && typeof err === 'object' && 'response' in err
          ...
          : undefined
      showToast(detail || 'Failed to fetch dependency impact', 'error')
    } finally {
  ...
  return (
    <div className="space-y-2 -mt-1">
      <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2">
        <AgentsHierarchyNav crumbs={AGENTS_NAV_CRUMBS} className="mb-0" />
      ...
      {agents.length === 0 ? (
        ...
        <div className="flex flex-col lg:flex-row gap-4 min-h-[calc(100vh-11rem)]">
          <AgentsListSidebar
            ...
            selectedRouteId={routeAgentId}
            selectedAgents={selectedAgents}
            isLoading={loading}
            onSelectAgent={handleSelectAgent}
            onSelectAgentCheckbox={handleSelectAgentCheckbox}
            onSelectAll={handleSelectAll}
          />
          <AgentWorkspaceDetail
            key={routeAgentId ?? 'none'}
            agentRouteId={routeAgentId}
            onAgentDeleted={handleAgentDeletedFromDetail}
              ...
              <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-amber-800 mb-2">Bulk delete operation</p>
                    <p className="text-xs text-amber-700">
                      Force delete will remove dependent records for all selected agents. Review impact
                      before confirming.
                    </p>
                  </div>
                </div>
              ...
              <div className="flex items-start gap-4 mb-6">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-700 mb-2">
                    ...
                    <span className="font-semibold text-gray-900">{selectedAgents.size}</span> selected
              ...
              <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-amber-700">
                      Force delete removes agents and dependent records. Call recordings are unlinked, not
                      deleted.
                    </p>
                  </div>
                </div>
