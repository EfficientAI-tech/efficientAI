...
type SourceType = 'transcript' | 'metric_output' | 'csv'
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      return apiClient.createJudgeDataset({
        name,
        description: description || undefined,
        source_type: 'metric_output',
        source_config: agentId ? { agent_id: agentId } : {},
      })
    },
    onSuccess: (dataset) => {
        ...
        <div className="p-6 space-y-5 overflow-y-auto">
          <div>
            ...
            <div className="mt-2 grid grid-cols-3 gap-2">
              ...
              />
              <SourceCard
                active={sourceType === 'metric_output'}
                icon={<Database className="h-5 w-5" />}
                title="Metric outputs"
                hint="Wrap existing conversation evals"
                onClick={() => setSourceType('metric_output')}
              />
            </div>
          </div>
          ...
          {sourceType === 'metric_output' && (
            <div>
              ...
              <select
                ...
                className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              >
                <option value="">All agents in this org</option>
                {agents.map((a: any) => (
                  <option key={a.id} value={a.id}>
                    {a.name}
                  </option>
              ...
              <p className="mt-1 text-xs text-gray-500">
                Imports up to 200 of the most recent ConversationEvaluation
                rows. Each becomes a sample with the evaluator's verdict
                shown as the "output".
              </p>
            </div>
          )}
