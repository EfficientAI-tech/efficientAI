...
def test_cron_dispatcher_routes_to_usage_queue():
    routes = celery_app.conf.task_routes
    assert routes["dispatch_cron_jobs"]["queue"] == USAGE_WORKER_QUEUE


def test_run_cron_evaluator_job_routes_to_default_worker():
    routes = celery_app.conf.task_routes
    assert routes["run_cron_evaluator_job"]["queue"] == "celery"
