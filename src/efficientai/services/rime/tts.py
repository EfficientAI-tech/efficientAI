...
from efficientai.services.tts_service import AudioContextWordTTSService, TTSService
from efficientai.transcriptions.language import Language, resolve_language
from efficientai.utils.text.base_text_aggregator import BaseTextAggregator
from efficientai.utils.text.skip_tags_aggregator import SkipTagsAggregator
...
try:
    ...
    logger.error("In order to use Rime, you need to `pip install efficientai-ai[rime]`.")
    raise Exception(f"Missing module: {e}")


def language_to_rime_language(language: Language) -> str:
    """Convert efficientai Language to Rime language code.

    Args:
        language: The efficientai Language enum value.
