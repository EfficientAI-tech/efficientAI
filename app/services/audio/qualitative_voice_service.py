...
class QualitativeVoiceMetricsService:
    ...
    def calculate_speaker_consistency(self, audio_path: str, segment_duration: float = 5.0) -> Optional[float]:
        ...
        try:
            ...
            if total_duration < segment_duration * 2:
                logger.warning("[QualitativeVoice] Audio too short for speaker consistency check")
            ...
            similarity = np.dot(start_emb, end_emb) / (np.linalg.norm(start_emb) * np.linalg.norm(end_emb))
    ...
    def calculate_metrics(self, audio_source: str, metric_names: List[str], is_url: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            # Get audio file path
            if is_url:
                from app.services.audio.voice_quality_service import download_audio
...
def calculate_qualitative_metrics(audio_source: str, metric_names: List[str], is_url: bool = True) -> Dict[str, Any]:
    """
