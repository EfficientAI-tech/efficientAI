...
class TranscriptionService:
        ...
        if db:
            try:
                ...
                audio_file = db.query(AudioFile).filter(
                    (AudioFile.file_path == audio_file_key) | (AudioFile.file_path.like(f"%{audio_file_key}%"))
                ).first()
    ...
    def _get_pyannote_pipeline(self):
        ...
        self._pyannote_pipeline = Pipeline.from_pretrained("pyannote/speaker-diarization-3.1", token=hf_token)
    ...
    def _detect_speakers_with_pyannote(
        ...
        logger.info(
            f"Running pyannote diarization on {audio_file_path} " f"(speaker hints: {pipeline_kwargs or 'auto'})"
        )
        ...
        if hasattr(raw_output, "itertracks"):
            ...
            raise TypeError(
                f"Unexpected pyannote output type: {type(raw_output).__name__}. " f"Available attributes: {attrs}"
            )
