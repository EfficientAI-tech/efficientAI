...
class TestAgentBridgeService:
    ...
    async def bridge_test_agent_to_voice_agent(
        ...
        voice_bundle = (
            ...
            .filter(VoiceBundle.id == agent.voice_bundle_id, VoiceBundle.organization_id == organization_id)
            .first()
        )
        if not voice_bundle:
        ...
        try:
            logger.info("[Bridge] Calling provider.create_web_call()...")
            ...
            logger.info("[Bridge] Step 2: Updating evaluator result status to CALL_INITIATING")
            ...
            logger.info("[Bridge] Initiating WebRTC bridge connection")
            ...
            try:
                logger.info("[Bridge] Waiting for bridge task to complete...")
                ...
                logger.info("[Bridge] Waiting for poll task to fetch results and trigger evaluation...")
                ...
                try:
                    ...
                    logger.info("[Bridge] Poll task was cancelled")
    ...
    async def _connect_and_bridge_with_webrtc(
        ...
        async def update_status(new_status: str, event: str = None, error: str = None):
            ...
            try:
                result = status_db.query(EvaluatorResult).filter(EvaluatorResult.id == evaluator_result_id).first()
                if result:
        ...
        try:
            ...
            await update_status(EvaluatorResultStatus.CALL_CONNECTING.value, "call_connecting")
            ...
            if provider_platform == "retell":
                webrtc_bridge = RetellWebRTCBridge(call_id=call_id, access_token=access_token, sample_rate=sample_rate)
                webrtc_bridge.on_call_ended = on_call_ended

                # Connect to Retell
                connected = await webrtc_bridge.connect_to_retell()
                if not connected:
                    await update_status(EvaluatorResultStatus.FAILED.value, "call_connection_failed", "Failed to connect to Retell WebRTC call")
                ...
                webrtc_bridge = VapiWebRTCBridge(call_id=call_id, web_call_url=web_call_url, sample_rate=sample_rate)
                webrtc_bridge.on_call_ended = on_call_ended

                # Connect to Vapi via Daily.co
                connected = await webrtc_bridge.connect_to_vapi()
                if not connected:
                    await update_status(EvaluatorResultStatus.FAILED.value, "call_connection_failed", "Failed to connect to Vapi WebRTC call")
                ...
                if not connected:
                    await update_status(EvaluatorResultStatus.FAILED.value, "call_connection_failed", "Failed to connect to ElevenLabs WebSocket")
                ...
                if webrtc_bridge.conversation_id:
                    ...
                    id_db = SessionLocal()
                    try:
                        r = id_db.query(EvaluatorResult).filter(EvaluatorResult.id == evaluator_result_id).first()
                        if r:
            ...
            await update_status(EvaluatorResultStatus.CALL_IN_PROGRESS.value, "call_started")
            ...
            voice_bundle = (
                db.query(VoiceBundle)
                .filter(VoiceBundle.id == voice_bundle_id, VoiceBundle.organization_id == organization_id)
                .first()
            )
            ...
            if test_agent:
                ...
                async def send_audio_chunks(audio: bytes):
                    ...
                    # ElevenLabs needs trailing silence so its VAD detects end-of-speech
                ...
                # ElevenLabs agents have a built-in greeting
                if provider_platform == "elevenlabs":
                    logger.info("[Bridge WebRTC] ElevenLabs: skipping first message — waiting for agent greeting")
                else:
    ...
    async def bridge_audio_streams(self, test_agent_ws: Any, voice_agent_call: Any) -> None:
        """
    ...
    async def _poll_call_results(
        ...
        try:
            result = poll_db.query(EvaluatorResult).filter(EvaluatorResult.id == evaluator_result_id).first()
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if call_status == "processing" and provider_platform == "elevenlabs":
                        logger.info("[Bridge Poll] ElevenLabs call still processing, waiting...")
                        continue
                    ...
                    if end_timestamp or call_status in ["ended", "completed", "failed", "done"]:
                        ...
                        poll_db.commit()
                        logger.info("[Bridge Poll] Status: CALL_ENDED")
                        ...
                        try:
                            ...
                            if plat == "elevenlabs":
                                ...
                                if audio_url:
                                    resp = _http.get(audio_url, headers={"xi-api-key": provider.api_key}, timeout=120)
                            ...
                            if audio_bytes:
                                ...
                                audio_s3_key = f"audio/organizations/{org_id}/evaluations/{result.provider_call_id}/{_uuid.uuid4()}.{ext}"
                                s3_service.upload_file_by_key(audio_bytes, audio_s3_key, content_type=content_type)
                                result.audio_s3_key = audio_s3_key
                                logger.info(f"[Bridge Poll] ✅ Uploaded call audio to S3: {audio_s3_key} ({len(audio_bytes)} bytes)")
                            else:
                                logger.warning(f"[Bridge Poll] Could not download audio for call {result.provider_call_id}")
                        except Exception as audio_err:
                            logger.warning(f"[Bridge Poll] Audio download/upload failed: {audio_err}")

                        # Commit all the data
                        poll_db.commit()
                        logger.info("[Bridge Poll] ✅ All call data committed to database")
                        ...
                        if transcript_text:
                            ...
                            result.error_message = None
                            poll_db.commit()
                            logger.info("[Bridge Poll] Status: EVALUATING")
                            ...
                            poll_db.commit()
                            logger.warning("[Bridge Poll] ❌ No transcript in call_data")
    ...
    def _extract_transcript_from_call_data(self, call_data: dict, provider_platform: str) -> tuple[str, list[dict]]:
        """
                ...
                for msg in transcript_object:
                    ...
                    if content.strip():
                        speaker_segments.append(
                            {"speaker": speaker, "text": content.strip(), "start": start_time, "end": end_time}
                        )
    ...
    def record_conversation(
        ...
        file_id = uuid.uuid4()
        s3_key = s3_service.upload_file(file_content=audio_data, file_id=file_id, file_format="wav")
