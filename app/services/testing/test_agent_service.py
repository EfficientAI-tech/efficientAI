...
class TestAgentService:
    ...
    def process_audio_chunk(
        ...
        # Get conversation
        conversation = (
            db.query(TestAgentConversation)
            .filter(
        ...
        voice_bundle = db.query(VoiceBundle).filter(VoiceBundle.id == conversation.voice_bundle_id).first()
        if not voice_bundle:
    ...
    def start_conversation(self, conversation_id: UUID, organization_id: UUID, db: Session) -> TestAgentConversation:
        """Start a conversation (change status to ACTIVE)."""
        conversation = (
            db.query(TestAgentConversation)
            .filter(TestAgentConversation.id == conversation_id, TestAgentConversation.organization_id == organization_id)
            .first()
        )
        if not conversation:
            raise ValueError(f"Conversation {conversation_id} not found")
    ...
    def end_conversation(
        ...
        if conversation.started_at:
            ...
            if started_at.tzinfo is None:
                started_at = started_at.replace(tzinfo=timezone.utc)
            conversation.duration_seconds = (ended_at - started_at).total_seconds()
