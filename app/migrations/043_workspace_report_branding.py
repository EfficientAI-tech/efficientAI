"""Migration: Ensure workspace-level report branding metadata exists.

Migration 042 introduced report branding while this feature was still moving
from organization scope to workspace scope. Some local databases may already
have 042 recorded without the final ``workspaces.report_branding`` column, so
this follow-up is intentionally idempotent and repairs that state.
"""

from sqlalchemy import text
from sqlalchemy.orm import Session

description = "Ensure workspaces.report_branding JSON metadata exists."
