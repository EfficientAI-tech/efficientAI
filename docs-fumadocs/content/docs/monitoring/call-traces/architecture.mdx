...
**Phase 2 (current):** Layout 3 thin API ingest (`defer_parse_to_worker: true`) stages raw OTLP in Postgres, `worker-traces` parses/correlates/persists batches, Celery derives turns, S3 offload on close. Set `defer_parse_to_worker: false` to parse in API (legacy Phase 2 path). Legacy sync JSONB ingest remains via `traces.async_ingest_enabled: false`.
