...
**Calls** in the sidebar shows production webhook calls and OTLP pipeline traces. For webhook ingest (Vapi, Retell, custom), see below. For **per-turn STT / LLM / TTS timing** from Pipecat bots, see [Call Traces](/docs/monitoring/call-traces/).
