...
auth:
  providers:
    ...
    - local_password
  local_password:
    # Lifetime of the short-lived access token minted at sign-in.
...
Once signed in, the SPA holds a short-lived access token (15 minutes by
default) plus a refresh token. The client silently refreshes the access
token before it expires. You can change lifetimes with `token_ttl_minutes`
and `refresh_token_ttl_days`. Logout revokes the refresh token and
...
> **Organization role is not the same as workspace role**
...
> **Why scoped tokens instead of an ambient `X-Organization-Id` header?**
