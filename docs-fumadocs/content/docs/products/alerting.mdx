...
Alerts are automatically evaluated every **5 minutes** by a Celery Beat schedule (`evaluate_alerts`, routed to the **`platform`** queue). You do not need to manually trigger evaluations — they run continuously as long as Beat and a `platform` queue worker are running.
...
Alert evaluation runs as a Celery Beat periodic task on the **`platform`** queue. Beat only enqueues tasks — you must also run a worker that consumes `platform`.
...
A default-queue worker does **not** process `evaluate_alerts`.
