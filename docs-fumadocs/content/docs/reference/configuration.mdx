...
# S3 (optional, used by the Data Sources feature when blob_provider is s3)
s3:
  enabled: false
...
# GCS (optional, used by the Data Sources feature when blob_provider is gcs)
gcs:
  enabled: false
  bucket_name: "your-gcs-bucket-name"
  project_id: "your-gcp-project-id"
  credentials_path: null     # service-account JSON; or GOOGLE_APPLICATION_CREDENTIALS / ADC
  signing_service_account_email: null  # optional override for IAM signBlob (GKE Workload Identity)
  prefix: "audio/"

# Azure Blob Storage (optional, used when blob_provider is azure)
azure:
  enabled: false
...
# Call traces / OTLP observability
traces:
  async_ingest_enabled: true
  defer_parse_to_worker: true    # S3 WAL + worker parse when clickhouse is set
...
# ClickHouse (call traces serving store; required for S3 WAL path)
clickhouse:
...
# LLM gateway (optional). Routes batch/eval LLM calls through a proxy.
# Per-org overrides in Integrations UI.
# llm_gateway:
#   enabled: false
...
:::tip
Keep `secret_key`, database credentials, S3/GCS credentials, and the license JWT
